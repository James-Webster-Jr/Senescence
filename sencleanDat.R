...
fileToLoad="chad_16sample_multipdResult_Proteins.txt"
...
outputFilename="sen.PD.LFQ_rawAbundances.NoCorrection.csv"
...
#Imported data for plex multiconsensus has 2658x131 rows x columns;
#2395 high+ confidence master proteins. RAW abundance is available
#for 16 samples in file.
...
cleanDat.0 = log(cleanDat.new, base = 2)
#remove infinite values
is.na(cleanDat.0) <- sapply(cleanDat.0, is.infinite)
cleanDat = cleanDat.0
library(readxl)
numericMeta <- read_xlsx("numericMeta.xlsx")

write.csv(cleanDat, file=outputFilename)
save(cleanDat, numericMeta, file = "SenescenceProteomicsCleanNumeric.RData")
